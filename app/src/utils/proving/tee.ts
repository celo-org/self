...
export async function firePayload(inputs: any, circuitName: string, timeoutMs = 1200000) {
    const uuid = v4();
    const ws = new WebSocket(WS_RPC_URL);
    let ws2: WebSocket | null = null;

    function createHelloBody(uuid: string) {
        return {
            jsonrpc: "2.0",
    ...
    ws.addEventListener("close", (event) => {
        console.log(`WebSocket 1 closed. Code: ${event.code}, Reason: ${event.reason}`);
    });
    ...
    function setupWs2(uuidVal: any) {
        ...
        ws2.addEventListener("close", (event) => {
            console.log(`WS2 closed. Code: ${event.code}, Reason: ${event.reason}, Clean: ${event.wasClean}`);
        });
    }
